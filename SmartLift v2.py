...
class Engine():
    ...
    def set_up(self):
        ...
        # set up dataframes
        self.exercise_score = self.exercises_data.loc[self.exercises_data["Active"] == 1, ["ID", "Exercise", "Type", "Unilateral", "Fitness", "Flexibility", "Compoundness", "Fatigue"]]
        self.exercise_score[["Freshness"]] = 0
        self.muscle_score = pd.DataFrame(list(self.exercises_data.columns[9:]), columns=["Muscle"])
    ...
    def constants(self):
        ...
        self.BASE_SETS_PER_WEEK = 10  # This can be adjusted between 4 and 12
        self.PIPELINE = []
        ...
        self.FITNESS_WEIGHT = 0.5          # FITNESS - prioritise cardio to improve fitness
        ...
        self.FATIGUE_WEIGHT = 0.5          # DIFFICULTY - try to have few but hard exercises (vs many easy ones)
        # Calculate CNS Fatigue Setpoint
        AVERAGE_EXERCISE_FATIGUE = 3  # Assuming average exercise fatigue
        WORKOUTS_PER_WEEK = 6
        SETS_PER_EXERCISE_INSTANCE = 3
        self.AVERAGE_EXERCISE_USAGE = 2.6
        priority_sum = self.muscle_targets_data["Priority"].sum()
        self.CNS_FATIGUE_SETPOINT = (self.BASE_SETS_PER_WEEK * priority_sum / WORKOUTS_PER_WEEK / SETS_PER_EXERCISE_INSTANCE * AVERAGE_EXERCISE_FATIGUE / self.AVERAGE_EXERCISE_USAGE)
    ...
    def CNS_fatigue_calc(self):
        for index, row in self.history.iterrows():  # loop through history records
            ...
            for i in range(10):  # loop through exercises completed
                ...
                fatigue_contrib = row2["Fatigue"].iloc[0] - max(0, 1 * is_easy)
                if row2["Type"].iloc[0] == "Cardio":
                    daily_fatigue = fatigue_contrib * 1  # Adding 100% of fatigue for Cardio
                else:
                    daily_fatigue = fatigue_contrib
                date_diff = (self.TODAY - dt.datetime.strptime(row["Date"], '%d/%m/%y').date()).days
                EMA_score = 0.3 * daily_fatigue * 0.7 ** (date_diff - 1)
    ...
    def overall_scores_calc(self):
        ...
        fatigue_best = self.exercise_score["Fatigue"].max()  # New best score for Fatigue
        fatigue_worst = self.exercise_score["Fatigue"].min()  # New worst score for Fatigue
        ...
        fn3 = lambda row: max(0,
                              ...
                              ** self.FLEXIBILITY_WEIGHT *
                              (1 + (row["Fatigue"] - fatigue_worst) / self.score_range_fix(fatigue_best, fatigue_worst))
                              ** self.FATIGUE_WEIGHT)  # New scoring component for Fatigue
    ...
    def generate_workout(self, date):
        ...
        # calculate fatigue cap and initialise self.remaining_fatigue, copy self.exercise_score
        self.fatigue_cap = round(self.CNS_FATIGUE_SETPOINT + min(3, self.CNS_FATIGUE_SETPOINT - self.EMA_CNS_FATIGUE), 1)
        self.remaining_fatigue = self.fatigue_cap
        self.exercise_score_init = self.exercise_score.copy()
        # print start matter
        print("Date: ", date)
        print("Current EMA CNS fatigue: ", round(self.EMA_CNS_FATIGUE, 1))
        print("Setpoint EMA CNS fatigue: ", round(self.CNS_FATIGUE_SETPOINT, 1))
        print("Fatigue cap: ", self.fatigue_cap)
        print("Old total balance:", round((self.muscle_score["Balance"] ** 2).sum(), 2))

        # Generate as many exercises as possible until the fatigue cap is reached
        while self.remaining_fatigue >= 1:
        ...
        print("New total balance:", round((self.muscle_score["Balance"] ** 2).sum(), 2))
    ...
    def select_top_extras(self, count):
        ...
        for index, row in top_exercises.iterrows():
            print("Extra option:", row["ID"], row["Exercise"], "(", row["Fatigue"], ")")

    def select_top_exercise(self):
        # Filter for CNS fatigue limits
        for index, row in self.exercise_score.iterrows():  # loop through all exercises
            if row["Fatigue"] > self.remaining_fatigue:
        ...
        fatigue_orig = row["Fatigue"].iloc[0]

        # Determine if using "easy" mode
        is_easy = fatigue_orig > self.remaining_fatigue
        fatigue_adj = max(0, fatigue_orig - 1) if is_easy else fatigue_orig
        ...
        if row["Type"].iloc[0] == "Cardio":
            self.remaining_fatigue -= fatigue_adj * 1  # Cardio contributes 100% to interim CNS fatigue.
        else:
            self.remaining_fatigue -= fatigue_adj

        # Write to list and remove from exercises
        self.PIPELINE.append(str(exercise_ID) + ("e" if is_easy else ""))
        self.exercise_score.drop([exercise_index], inplace=True)
        print("Selected", exercise_ID, row["Exercise"].iloc[0], "(", fatigue_adj, ")")
