...
class Engine():
    ...
    def constants(self):
        ...
        if self.SUNNY:
            ...
            self.TARGET_DURATION = 35  # Total exercise duration in minutes (weekends)
            ...
            self.PROPORTIONS_FACTOR = 0  # prioritise neglected muscles according to muscle_targets preferences (slightly less optimal)
            self.SYMMETRY_FACTOR = 0  # prioritise single-limb exercises fixing asymmetry (moderately less optimal)
            self.VARIETY_FACTOR = 0  # prioritise exercises you haven't done recently to mix things up (moderately less optimal)
            ...
            # cardio
            self.CALORIES_FACTOR = 0.5  # prioritise burning the most calories (optimal)
            self.VARIETY_FACTOR_C = 0  # prioritise exercises you haven't done recently to mix things up (moderately less optimal)
    ...
    def history_EMA_calc(self):
        ...
        for index, row in self.history.iterrows():  # loop through history records
            ...
            EMA_ratio_score += 0.1 * daily_lift_ratio * daily_total_duration * 0.9 ** (date_diff - 1)
    ...
    def new_deviation_calc(self, exercise_ID=None):
        ...
        exercises_to_calculate = self.exercise_score if exercise_ID is None else self.exercise_score[self.exercise_score["ID"] == exercise_ID]
        ...
        for index, row in exercises_to_calculate.iterrows():  # Loop through all (active) exercises
            ...
            for index2, row2 in self.muscle_score.iterrows():
                ...
                try:
                    ...
                    score = 0
                score = float(score)
                projected_sets_per_week = 0.9 * row2['Sets_per_week'] + 0.1 * score * 3 * 7
                self.muscle_score.loc[index2, ["Projected_Sets_per_week"]] = projected_sets_per_week
            ...
            self.muscle_score["Projected_Target"] = (self.muscle_targets_data["Priority"] / total_priority) * projected_avg_ema_sets_per_week * len(self.muscle_targets_data)

            # Calculate the projected deviation as the difference between the projected target and the projected sets per week
            self.muscle_score["New_deviation"] = self.muscle_score["Projected_Sets_per_week"] - self.muscle_score["Projected_Target"]

            # Calculate the RMS of the new deviations for the exercise
            RMS = (self.muscle_score["New_deviation"] ** 2).mean() ** 0.5 - (self.muscle_score["Deviation"] ** 2).mean() ** 0.5
    ...
    def overall_scores_calc(self):
        ...
        proportions_best = self.exercise_score["Deviation_change"].min()  # Lower is better
        ...
        def score_lift(row):
            return max(0,
                       (1 + (row["Growth"] - growth_worst) / self.score_range_fix(growth_best, growth_worst))
                       ** self.GROWTH_FACTOR *
                       (1 + (row["Deviation_change"] - proportions_worst) / self.score_range_fix(proportions_best, proportions_worst))
                       ** self.PROPORTIONS_FACTOR *
                       (1 + (row["Unilateral"] - symmetry_worst) / self.score_range_fix(symmetry_best, symmetry_worst))
                       ** self.SYMMETRY_FACTOR *
                       (1 + (row["Freshness"] - variety_worst) / self.score_range_fix(variety_best, variety_worst))
                       ** self.VARIETY_FACTOR *
                       (1 + (row["Active"] - favourites_worst) / self.score_range_fix(favourites_best, favourites_worst))
                       ** self.FAVOURITES_FACTOR *
                       (1 + (comfort_best - row["Fatigue"]) / self.score_range_fix(comfort_best, comfort_worst))
                       ** self.COMFORT_FACTOR)

        def score_cardio(row):
            return max(0,
                       (1 + (row["Calories"] - calories_worst) / self.score_range_fix(calories_best, calories_worst))
