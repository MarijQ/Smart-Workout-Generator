...
class Engine():
    ...
    def set_up(self):
        ...
        self.muscle_score['Sets_per_week'] = self.muscle_score['Sets_per_week'].astype(
            float)
        self.muscle_score["Workout_fatigue"] = self.muscle_score["Workout_fatigue"].astype(
            float)
    ...
    def prep_workout(self):
        ...
        if self.IS_HOME:
            ...
            self.lift_probability = min(self.TARGET_LIFT_RATIO + 0.2, max(
                self.TARGET_LIFT_RATIO - 0.2, (2 * self.TARGET_LIFT_RATIO - self.EMA_LIFT_RATIO)))
    ...
    def update_history_with_names(self):
        ...
        exercise_dict = pd.Series(
            self.exercises_data['Exercise'].values, index=self.exercises_data['ID']).to_dict()
    ...
    def history_EMA_calc(self):
        ...
        for index, row in self.history.iterrows():  # loop through history records
            ...
            for i in range(10):  # loop through exercises completed
                ...
                row2 = self.exercises_data[self.exercises_data["ID"]
                                           == exercise_ID]
            ...
            date_diff = (
                self.TODAY - dt.datetime.strptime(row["Date"], '%d/%m/%y').date()).days
            EMA_ratio_score += 0.1 * daily_lift_ratio * \
                daily_total_duration * 0.9 ** (date_diff - 1)
            EMA_calories_score += 0.1 * \
                daily_total_calories * 0.9 ** (date_diff - 1)
            EMA_counts += 0.1 * 1 * daily_total_duration * \
                0.9 ** (date_diff - 1)
    ...
    def freshness_calc(self):
        ...
        for index, row in self.exercise_score.iterrows():  # Loop through all exercises
            ...
            self.exercise_score.loc[index, ["Freshness"]] = round(
                scaled_freshness, 2)
        ...
        for index, row in self.history.iterrows():  # loop through history records
            for i in range(10):  # loop through exercises completed
                ...
                for index2, row2 in self.muscle_score.iterrows():  # add EMA contributions for each muscle
                    score = self.exercises_data.loc[self.exercises_data["ID"]
                                                    == exercise_ID, row2["Muscle"]].iloc[0]
                    try:
                        int(score)
                    except ValueError:
                        continue
                    exercise_type = self.exercises_data.loc[self.exercises_data["ID"]
                                                            == exercise_ID, "Type"].iloc[0]
                    score = float(score)
                    ...
                    EMA_score_fatigue = self.MUSCLE_FATIGUE_EMA_BIAS * fatigue_score * \
                        (1 - self.MUSCLE_FATIGUE_EMA_BIAS) ** date_diff if exercise_type != "Cardio" else 0
                    # #
                    # if index2 == 14:
                    #     print(date_diff, deviation_score, EMA_score_deviation)

                    self.muscle_score.loc[index2, [
                        'Sets_per_week']] += EMA_score_deviation * 3 * 7
                    # Ensure non-negative
                    self.muscle_score.loc[index2, [
                        "EMA_fatigue"]] += max(0, EMA_score_fatigue)
    ...
    def muscle_target_calc(self, eod=False):
        ...
        self.muscle_score["Target"] = (self.muscle_targets_data["Priority"] /
                                       total_priority) * avg_ema_sets_per_week * len(self.muscle_targets_data)

        # Calculate deviation as the absolute difference between Target and actual Sets_per_week
        self.muscle_score["Deviation"] = self.muscle_score['Sets_per_week'] - \
    ...
    def new_deviation_calc(self, exercise_ID=None):
        ...
        exercises_to_calculate = self.exercise_score if exercise_ID is None else self.exercise_score[
            self.exercise_score["ID"] == exercise_ID]

        for index, row in exercises_to_calculate.iterrows():  # Loop through all (active) exercises
            exercise_ID = row["ID"]
            exercise_type = self.exercises_data.loc[self.exercises_data["ID"]
                                                    == exercise_ID, "Type"].iloc[0]
            ...
            for index2, row2 in self.muscle_score.iterrows():
                ...
                try:
                    ...
                    score = 0
                score = float(score)
                projected_sets_per_week = 0.9 * \
                    row2['Sets_per_week'] + 0.1 * score * 3 * 7
                self.muscle_score.loc[index2, [
                    "Projected_Sets_per_week"]] = projected_sets_per_week

            # Calculate the projected mean EMA sets per week
            projected_avg_ema_sets_per_week = self.muscle_score["Projected_Sets_per_week"].mean(
            )

            # Calculate the projected target for each muscle based on its priority
            total_priority = self.muscle_targets_data["Priority"].sum()
            self.muscle_score["Projected_Target"] = (
                self.muscle_targets_data["Priority"] / total_priority) * projected_avg_ema_sets_per_week * len(self.muscle_targets_data)

            # Calculate the projected deviation as the difference between the projected target and the projected sets per week
            self.muscle_score["New_deviation"] = self.muscle_score["Projected_Sets_per_week"] - \
                self.muscle_score["Projected_Target"]

            # Calculate the RMS of the new deviations for the exercise
            RMS = (self.muscle_score["New_deviation"] ** 2).mean() ** 0.5 - \
                (self.muscle_score["Deviation"] ** 2).mean() ** 0.5
    ...
    def generate_workout(self, date, extras):
        ...
        expected_cardio_time = (
            1 - self.lift_probability) * self.TARGET_DURATION
        ...
        while self.remaining_duration >= 7:  # Assuming minimum exercise duration is 7 minutes
            ...
            if reduce_cardio_prob:
                ...
                selected_row = self.exercises_data[self.exercises_data["ID"]
    ...
    def select_top_exercise(self):
        ...
        self.exercise_score['First_Word'] = self.exercise_score['Exercise'].apply(
            lambda x: x.split()[0])
        self.exercise_score = self.exercise_score[~self.exercise_score['First_Word'].isin(
            self.selected_first_words)]
        ...
        if self.exercise_score.empty:
            print("No more suitable exercises!")
            return False
        row = self.exercise_score.sort_values(
            by="Score", ascending=False).head(1)
        try:
        ...
        print("Selected", int(exercise_ID),
              row["Exercise"].iloc[0], "(", int(duration), "mins )")
    ...
    def select_top_extras(self, count):
        ...
        filtered_exercises = self.exercise_score_init[~self.exercise_score_init['ID'].isin(
            [int(x) for x in self.PIPELINE])]
        ...
        top_exercises = filtered_exercises.sort_values(
            by="Score", ascending=False).head(count)

        for index, row in top_exercises.iterrows():
            print("Extra option:", int(
                row["ID"]), row["Exercise"], "(", int(row["Duration"]), "mins )")
    ...
    def decode(self, exercise_str):
        ...
        if match:
            ...
            exercise_name = self.exercises_data.loc[self.exercises_data['ID']
    ...
    def forecast(self, offset, days, options):
        ...
        for i in range(days):
            ...
            if "exercises" in options:
                print(
                    self.exercise_score[["Exercise", "Freshness", "Deviation_change", "Score"]])
            if "stats" in options:
            ...
            if "muscles" in options and i == 0:
                print(self.muscle_score[["Muscle", "Workout_fatigue", "EMA_fatigue", 'Sets_per_week',
                      "Target", "Deviation"]].sort_values(by="Deviation", ascending=True))
