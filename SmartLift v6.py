...
class Engine:
    ...
    def set_up(self):
        ...
        self.history = pd.read_excel(
            f"Smart-Workout-Generator-[GH]/inputs/history{suffix}.ods", engine="odf", sheet_name="history"
        )
        self.exercises_data = pd.read_excel(
            f"Smart-Workout-Generator-[GH]/inputs/exercises{suffix}.ods",
            engine="odf",
            sheet_name="exercises",
            skiprows=1,
        )
        self.muscle_targets_data = pd.read_excel(
            f"Smart-Workout-Generator-[GH]/inputs/muscle_targets{suffix}.ods",
            engine="odf",
    ...
    def filter_out_today_entries(self):
        ...
        suffix = "_s" if self.SUNNY else ""
        with pd.ExcelWriter(f"Smart-Workout-Generator-[GH]/inputs/history{suffix}.ods", engine="odf") as writer:
    ...
    def write_to_file(self, date):
        try:
            with open("Smart-Workout-Generator-[GH]/inputs/history.csv", "a", newline="") as my_csv:
